...
CakeBazar is currently in **Phase 0 validation**. It is not a production marketplace.

The active product hypothesis is a trust-first, managed request workflow for custom cakes:
...
Do not implement future ecosystem ideas unless the task explicitly says they have been unlocked by evidence.
...
Existing older documents may be aspirational, duplicated, mixed-language, or superseded. When they conflict with the files above, the accepted Founder Reset documents are the current source of truth.
...
User-facing copy must be localization-ready. Do not duplicate business-critical copy across components when a shared content layer is appropriate.
...
Before submitting changes in `apps/web`, run:
